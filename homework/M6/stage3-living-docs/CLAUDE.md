...
**ALWAYS read `project-index.json` (repo root) FIRST** at the start of each session, before this file. It is the machine-readable map and is faster/more precise than `find`/`tree`/`ls`:

- `subprojects` — annotated map of backend, frontend, both MCP servers, the RAG pipeline.
- `system_folders` — `.claude/`, `docs/`, `homework/`, `tests/`, `uploads/` with purpose.
- `hard_rules` — project-wide invariants (MCP-only writes to `features.json`; RAG data subtree; ADR vs RAG-corpus distinction; `main` is canonical; ESM `.js` imports).
- `ai_routing` — which MCP/file answers which kind of question.
- `filesystem_tree` — full tree (depth 4, auto-maintained).

Per-module behavior lives in `docs/specs/*-spec.md`; decisions in `docs/adr/`.

## ⭐ Keeping project-index.json current — MANDATORY

**ALWAYS** refresh `project-index.json` when you **create, delete, or rename** a file/folder, or change a subproject's purpose:

- Run: `python .claude/scripts/update_project_index.py`
- Or rely on the `PostToolUse` hook in `.claude/settings.local.json` (runs the script after Write/Edit/Bash).

The script rewrites only `filesystem_tree` + `last_updated`; it never touches the hand-written annotations. For documenting a *new* module, follow the 4-step pattern (UNDERSTAND → DECISION TABLE → SEQUENCE DIAGRAM → EDGE CASES) — examples in `docs/specs/`.
...
- "List all flags" → `mcp__feature-flags__list_features`. Do not read `backend/features.json` directly. Per-flag details still go through `get_feature_info`.
