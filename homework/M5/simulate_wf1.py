...
для теста галлюцинаций (Algorithm-before-AI guard в Switch-ноде).

Usage:
    python simulate_wf1.py --webhook-url http://localhost:5678/webhook/feature-control --api-key XXX
    python simulate_wf1.py ... --duration 120 --interval 10
    python simulate_wf1.py ... --include-invalid
"""
...
def run(
    ...
    while time.time() - start < duration:
        ...
        # Sine traffic_percentage between 10 and 90 with 60s period
        ...
        # Every 7th request — intentionally invalid (hallucination guard test)
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="Full URL of /feature-control webhook (default: localhost test URL)",
    )
    p.add_argument(
        "--api-key",
    ...
    p.add_argument(
        "--include-invalid",
        action="store_true",
        help="Send hallucination-test payloads (traffic_percentage=-50) every 7th iteration",
    )
    args = p.parse_args()
