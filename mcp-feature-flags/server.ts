...
if (useHttp) {
  const port = Number(process.env.MCP_HTTP_PORT ?? 5680);
  const app = express();
  app.use(express.json({ limit: "4mb" }));

  // Request tracer — logs every hit with method/path/session-id/body-method.
  app.use((req, _res, next) => {
    const sid = req.header("mcp-session-id") || "-";
    const accept = req.header("accept") || "-";
    const bodyMethod = req.body && typeof req.body === "object" ? (req.body as any).method : undefined;
    console.log(`[${new Date().toISOString()}] ${req.method} ${req.path} sid=${sid} accept="${accept}" body.method=${bodyMethod ?? "-"}`);
    next();
  });

  // Stateless POST: fresh McpServer + transport per request. Fast handshake-free
  // tool calls. n8n's MCP Client Tool tolerates stateless mode (it just doesn't
  ...
  // Legacy SSE transport. Some MCP clients (incl. n8n's MCP Client Tool when
  // not in Streamable HTTP mode) start with GET, expecting an `event: endpoint`
  // message that tells them where to POST JSON-RPC messages. We honour it via
  // SSEServerTransport — a separate session per GET stream, dispatched by
  // sessionId query param on POST /mcp/message.
  ...
  app.get("/mcp", async (req, res) => {
    try {
      ...
      req.on("close", () => { transport.close().catch(() => {}); });
      await server.connect(transport);
    } catch (e: any) {
      if (!res.headersSent) res.status(500).end(`SSE setup error: ${e?.message ?? e}`);
    }
  });
  ...
  app.post("/mcp/message", async (req, res) => {
    ...
    if (!transport) {
      res.status(404).json({ error: "UNKNOWN_SESSION", sessionId });
      return;
    }
    try {
  ...
  // DELETE — Streamable HTTP "close session". Stateless POST handler has no
  // sessions, so this is a no-op for compatibility.
  app.delete("/mcp", (_req, res) => res.status(200).end());

  void randomUUID;

  app.get("/health", (_req, res) => res.json({ ok: true, transport: "http", endpoint: "/mcp" }));

  // Read-only REST shortcut for HTTP clients that can't do the Streamable HTTP
  // handshake (e.g. plain n8n HTTP Request node). Mirrors get_feature_info
  // output but without the JSON-RPC envelope or session requirement.
  app.get("/feature/:feature_name", async (req, res) => {
    try {
      const data = await load();
      const f = data[req.params.feature_name];
      if (!f) {
        res.status(404).json({ error: "FEATURE_NOT_FOUND", feature_name: req.params.feature_name });
        return;
      }
      res.json({ feature_name: req.params.feature_name, ...f, dependencies_state: depsState(data, f) });
    } catch (e: any) {
  ...
  app.listen(port, () => {
    console.log(`MCP feature-flags listening on http://localhost:${port}/mcp (Streamable HTTP, stateless)`);
  });
} else {
  const server = buildServer();
